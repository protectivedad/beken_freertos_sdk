...
#define CLIENT_THREAD_PROIRITY  4
...
static uint32_t iperf_client_priority = CLIENT_THREAD_PROIRITY;
static uint32_t iperf_server_priority = SERVER_THREAD_PROIRITY;
...
static void iperf_udp_server(void *thread_param)
{
    ...
    while (s_param.state == IPERF_STATE_STARTED) {
        ...
        while (s_param.state == IPERF_STATE_STARTED) {
            ...
            if (r_size > 12) {
                iperf_report_init();
                goto has_receive_udp_pkt;
                //break;
            }
        }

        while (((s_param.state == IPERF_STATE_STARTED) && (tick2 - tick1) < (TICK_PER_SECOND * 9999))) {
            r_size = recvfrom(sock, buffer, IPERF_BUFSZ_UDP, 0, (struct sockaddr *)&sender, (socklen_t *)&sender_len);
has_receive_udp_pkt:
            if (r_size > 12) {
...
void iperf_usage(void)
{
    ...
    os_printf("  config -server_pri/-client_pri 	  set iperf thread priority(0-9, 0 is highest)\n");
    return;
}
...
static void iperf_start(int mode, char *host, int port, uint32_t duration)
{
    if (s_param.state == IPERF_STATE_STOPPED) {
        ...
        if (mode == IPERF_MODE_TCP_CLIENT) {
            rtos_create_thread(NULL, iperf_client_priority, "iperf_tcp_c",
                               iperf_client, THREAD_SIZE,
                               (beken_thread_arg_t) 0);
        } else if (mode == IPERF_MODE_TCP_SERVER) {
            rtos_create_thread(NULL, iperf_server_priority, "iperf_tcp_s",
                               iperf_server, THREAD_SIZE,
                               (beken_thread_arg_t) 0);
        } else if (mode == IPERF_MODE_UDP_CLIENT) {
            rtos_create_thread(NULL, iperf_client_priority, "iperf_udp_c",
                               iperf_udp_client, THREAD_SIZE,
                               (beken_thread_arg_t) 0);
        } else if (mode == IPERF_MODE_UDP_SERVER) {
            rtos_create_thread(NULL, iperf_server_priority, "iperf_udp_s",
...
void iperf_config(int argc, char **argv)
{
    ...
    if (os_strcmp(argv[2], "-server_pri") == 0) {
        iperf_server_priority = os_strtoul(argv[3], NULL, 10);
        os_printf("iperf config iperf_server_priority to %d !\n", iperf_server_priority);
    } else if (os_strcmp(argv[2], "-client_pri") == 0) {
        iperf_client_priority = os_strtoul(argv[3], NULL, 10);
        os_printf("iperf config iperf_client_priority to %d !\n", iperf_client_priority);
    } else {
